...
FIXED_SPECIAL_DAYS: list[tuple[str, str, str]] = [
    ...
    ("05-20", "告白日", "听起来有点害羞，但亚托莉会记得这个日子。"),
    ("12-25", "圣诞节", "年底温柔又热闹的一天，亚托莉想和你一起记住。"),
]
...
def day_note(session: Session, *, day: str) -> str:
    ...
    if experiences:
        ...
        return f"亚托莉回想这一天：{summaries}" if summaries else ""
    try:
        event_day = date.fromisoformat(day)
    except ValueError:
        return ""
    if event_day < date.today():
        return "这一天已经过去了，亚托莉会把它当作一页安静的回忆。"
    if event_day == date.today():
        return "今天是特别的一天，亚托莉想把心情认真留在这里。"
    return "这一天还没到，亚托莉对它有一点小小的期待。"
